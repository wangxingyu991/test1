...
<template>
  <div class="drawer-wrap">
      <div class="rh-drawer-head">
          <div class="drawer-title">新增发票票样</div>
      </div>
...
import validate from "@/utils/validate.js";
 ...
 export default {
    ...
    methods: {
      ...
      selectSf(){
        let _this=this;
        let sf=_this.add_Form.sf;
        let py_title=sf+"增值税电子普通发票"
        _this.add_Form.pymc=py_title;
        $('#div_title').html(py_title);
      },
      //初始化设置标签属性值
      previewInitImg(){ 
        let _this=this;
    ...
    setPrerviewImg(imgrul){
      ...
      handlechange(file,fileList){
       ...
       let uid = file.uid
       let isLt2M = file.size / 1024 / 30 < 1;   //不能超过30kb
...
               }
              ...
              _this.$api.post(
                _this.$apis.icpmanagement + "/basic/invoicelayout/saveInvoicelayout", paraForm,
                function (res) {
                  console.log("res="+res);
                if (res.success) {
                    _this.$message({
