...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        jobName: [
          { required: true, message: "请输入任务名称", transform: value => value ? value.trim() : '', trigger: "blur" },
          //{ max: 100, message: "最多100个字符", trigger: "blur" },
        ],
        jobGroup: [
          { required: true, message: "请输入任务组名称", transform: value => value ? value.trim() : '',trigger: "blur" },
          //{ max: 100, message: "最多100个字符", trigger: "blur" },
        ],
        jobClassName: [
          { required: true, message: "请输入任务全路径", transform: value => value ? value.trim() : '',trigger: "blur" },
          //{ max: 3000, message: "最多3000个字符", trigger: "blur" },
        ],
        cronExpression: [
          { required: true, message: "请输入任务表达式", transform: value => value ? value.trim() : '',trigger: "blur" },
    ...
    onSubmit(formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          this.$api.get(
            _this.$apis.icptask + "/rest/job/cron?cron="+editParams.cronExpression, {},
            function (res) {
              if (res.success) {
                    if(!_this.editFlag){
                      let arr = ["group","by"];
                      if (editParams != null) {
                        if (
                          validate.checkForbiddenWordFormIgnore(JSON.stringify(editParams),arr)
                        ) {
                          _this.$message({
                            showClose: true,
                            message: "不允许有特殊字符",
                    ...
                    //修改
                    _this.$api.post(
                      _this.$apis.icptask + "/rest/job/reschedulejson/", editParams,
                      function (res) {
                          //关闭loading
                          _this.loading=false;
                          if (res.success) {
                    ...
                    if (editParams != null) {
                       let arr = ["group","by"];
                        if (validate.checkForbiddenWordFormIgnore(JSON.stringify(editParams),arr)) {
                          _this.$message({
                            showClose: true,
                            message: "不允许有特殊字符",
                            type: "error",
                    ...
                    _this.loading=true;
                    _this.$api.post(
                      _this.$apis.icptask + "/rest/job/addjson/", editParams,
                      //_this.$apis.icpmanagement + '/taxcontrol/terminal/add?jobName='+123+'&jobClassName='+456,{},
                      //_this.$apis.icptask + "/rest/job/add?jobName=1&jobClassName=1&jobClassName=1&cronExpression=22", {},
                      //_this.$apis.icptask + "/rest/job/query", editParams,
                      function (res) {
                          //关闭loading
                          _this.loading=false;
                          if (res.success) {
