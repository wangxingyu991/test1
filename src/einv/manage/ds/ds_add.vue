<template>
  <div class="drawer-wrap ds_add">
    <div class="rh-drawer-head">
      <div class="drawer-title">新增数据源</div>
    </div>
            ...
            <el-row>
                <el-col :span="24">
                    <el-form-item label="数据源类别" prop="dsType">
                        <el-radio-group v-model.trim='operatorForm.dsType' @change="radioChange(operatorForm.dsType)">
      ...
      if(_this.operatorForm.dsType == "0"){
        ...
        _this.$refs.operatorForm.validate((valid) => {
          if (valid) {
            ...
            _this.loading = true;
            _this.$api.post(this.$apis.manage+"/rest/manage/ds/saveds", para, function (res) {
              _this.loading = false;
              if (res.success == true) {
                _this.$message({
                  message: '保存成功',
        ...
        if(this.tableShardingInfo.length <= 0){
          ...
          if (valid) {
            ...
            let para = Object.assign({}, _this.operatorForm);
            _this.loading = true;
            _this.$api.post(this.$apis.manage+"/rest/manage/ds/saveshardingds", para, function (res) {
              _this.loading = false;
              if (res.success == true) {
                _this.$message({
                  message: '保存成功',
