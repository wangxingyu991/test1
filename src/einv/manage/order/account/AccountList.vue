<template>
  <div>
    <div class="rh-page-header">
      <div class="rh-head-and-operate">
        <div class="tips-content">
          <span class="rh-helpTip-text">收款账户配置</span>
        </div>
      </div>
    </div>
    <div class="rh-page-content">
        ...
        <div class="rh-pull-right">
          <el-form :model="queryForm" ref="queryForm" :inline="false" label-width="0px" label-position="right">
            <el-form-item prop="accountName">
              <el-input v-model="queryForm.accountName" placeholder="账户名称" maxlength="100"></el-input>
            </el-form-item>
      ...
      <div class="rh-table">
        <el-table :data="tableData.records" stripe border @selection-change="handleSelectionChange" v-loading="loading">
          <el-table-column show-overflow-tooltip align="left" prop="accountName" label="账户名称"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="accountNo" label="账号"></el-table-column>
          <el-table-column show-overflow-tooltip align="left" prop="accountAddress" label="开户行地址"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="accountTel" label="联系方式" width="120"></el-table-column>
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <el-button class="table_btn" type="text" @click="onUpdate(scope.row.accountId)">修改</el-button>
              <el-button class="table_btn" type="text" v-if="scope.row.statue === '有效'" @click="changeStatus(scope.row.accountId,'N')">禁用</el-button>
              <el-button class="table_btn" type="text" v-if="scope.row.statue === '无效'" @click="changeStatus(scope.row.accountId,'Y')">启用</el-button>
              <el-button class="table_btn" type="text" @click="delate_fn(scope.row.accountId)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <div class="dailog">
      <el-dialog title="新增收款账户" :visible.sync="add_Dialog" width="50%" append-to-body>
        <AddPage v-if="add_Dialog" @back="back" :addPageData="addPageData"></AddPage>
      </el-dialog>
    </div>
    <div class="dailog">
      <el-dialog title="修改收款账户" :visible.sync="update_Dialog" width="50%" append-to-body>
...
<script>
import AddPage from "./AccountAdd.vue";
import UpdatePage from "./AccountUpdate.vue";
...
export default {
  ...
  methods: {
    ...
    init_Table() {
      ...
      _this.loading = true;
      this.$api.post(
        _this.$apis.manage + "/manage/order/account/listAccount/",
        params,
        function (res) {
          if (res.success) {
    ...
    delate_fn(accountId) {
      ...
      _this
        ...
        .then(() => {
          _this.$api.delete(
            _this.$apis.manage +
            "/manage/order/account/deleteAccount/" +
            accountId,
            {},
            function (res) {
              if (res.success) {
    ...
    onAdd() {
      let _this = this;
      _this.$api.get(
        _this.$apis.manage + "/manage/order/account/checkOnly",
        {},
        function (res) {
          if (res.success == true) {
      ...
      _this.$api.get(
        ...
        function (res) {
          if (res.success == true || status == "N") {
            _this.$api.post(
              _this.$apis.manage +
              "/manage/order/account/changeStatus/" +
              accountId,
              { status: status },
              function (res) {
                if (res.success == true) {
