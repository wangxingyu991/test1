<template>
  <div class="drawer-wrap">
    <div class="rh-drawer-head">
      <div class="drawer-title">财务审核 - {{showName}}</div>
    </div>
      ...
      <el-collapse v-model="activeNames">
        <el-collapse-item title="订单信息" name="1">
          <div>
            <el-row>
              <el-col :span="8">
                <span class="name">
                  订单号：{{reviewData.orderId}}
                </span>
              </el-col>
              <el-col :span="8">
                <span class="name">
                  订单类型：{{reviewData.orderTypeName}}
                </span>
              </el-col>
              <el-col :span="8">
                <span class="name">
                  订单状态：{{reviewData.orderStatusName}}
                </span>
            ...
            <el-row>
              <el-col :span="8">
                <span class="name">
                  应付金额：{{reviewData.orderTotalMoney}}元
                </span>
              </el-col>
              <el-col :span="8">
                <span class="name">
                  折扣金额：{{reviewData.discountPrice}}元
                </span>
              </el-col>
              <el-col :span="8">
                <span class="name">
                  满减金额：{{reviewData.preferentialPrice}}元
                </span>
              </el-col>
            </el-row>
            <el-row>
        ...
        <el-collapse-item title="支付信息" name="2">
          <el-row>
            <el-col :span="8">
              <span class="name">
                支付方式：{{reviewData.payMethodName}}
              </span>
            </el-col>
            <el-col :span="8">
              <span class="name" v-if="reviewData.payMethod == '1'||reviewData.payMethod == '2'">
                支付流水号：{{payNo}}
              </span>
            </el-col>
            <el-col :span="8">
              <span class="name">
                支付时间：{{reviewData.paymentTime}}
              </span>
            </el-col>
            <el-col :span="8" v-if="reviewData.payMethod == '3'">
              <span class="name">
                入账金额：{{reviewData.orderTotalMoney}}元
              </span>
            </el-col>
          </el-row>
          <el-row v-if="reviewData.payMethod == '1'||reviewData.payMethod == '2'">
            <el-col :span="8" v-if="reviewData.payMethod == '2'">
              <span class="name">
                微信账号：{{wechatId}}
              </span>
            </el-col>
            <el-col :span="8" v-if="reviewData.payMethod == '1'">
              <span class="name">
                支付宝账号：{{alipayId}}
              </span>
            </el-col>
          ...
          <el-row v-if="reviewData.payMethod == '3'">
            <el-col :span="8">
              <span style="color:red;margin-left: -9px;">*</span>
              <span class="name">
                账&#12288;&#12288;号：
              </span>
              <el-input size="mini" v-model="accountNo" placeholder="账号" style="width: 180px;" maxlength="50"></el-input>
            </el-col>
            <el-col :span="8">
              <span style="color:red;margin-left: -9px;">*</span>
              <span class="name">
              ...
              <el-input size="mini" v-model="accountName" placeholder="账户名称" style="width: 180px;" maxlength="100"></el-input>
            </el-col>
            <el-col :span="8">
              <span style="color:red;margin-left: -9px;">*</span>
              <span class="name">
              ...
              <el-input size="mini" v-model="phone" placeholder="联系方式" style="width: 180px;" maxlength="20"></el-input>
            </el-col>
          </el-row>
        </el-collapse-item>
        <el-collapse-item title="财务审核" name="3">
          <el-col :span="20">
            ...
            <el-upload action="1" list-type="picture-card" ref="upload" :limit="1" accept="image/jpeg,image/gif,image/png" :multiple="false" :on-change="handleFileUpload" :file-list="this.fileList"
              :auto-upload="false" style="margin-left: 68px;">
          ...
          <el-col :span="24">
            ...
            <el-input type="textarea" size="mini" v-model="operateIdea" placeholder="审核意见" maxlength="200"></el-input>
          </el-col>
        </el-collapse-item>
      </el-collapse>
    </div>
...
export default {
  ...
  methods: {
    ...
    details() {
      let _this = this;
      let orderId = _this.reviewData.orderId;
      if (_this.reviewData.orderType == "0" || _this.reviewData.orderType == "2") {//0、产品订单2、续费订单
        _this.isProduct = true;
        _this.productData = {
          orderId: orderId,
          orderType: _this.reviewData.orderType,
        };
      } else if (_this.reviewData.orderType == "1") {//1、短信包订单
        _this.$api.post(
    ...
    getDate() {
      let _this = this;
      if (_this.reviewData.orderType == "0") {
        _this.showName = "产品购买";
      } else if (_this.reviewData.orderType == "1") {
        _this.showName = "短信包购买";
      } else if (_this.reviewData.orderType == "2") {
        _this.showName = "续费";
      }
      _this.$api.get(
        _this.$apis.manage + "/manage/order/financeReview/getData/" +
        _this.reviewData.orderId,
        {},
        function (res) {
          if (res.success) {
    ...
    confirm() {
      ...
      if (_this.check()) {
        _this.loading = true;
        let params = {};
        _this.$set(params, "orderId", _this.reviewData.orderId);
        _this.$set(params, "workOrderSystemId", _this.reviewData.workOrderSystemId);
        _this.$set(params, "accountNo", _this.accountNo);
        _this.$set(params, "accountName", _this.accountName);
        _this.$set(params, "phone", _this.phone);
        _this.$set(params, "operateIdea", _this.operateIdea);
        _this.$set(params, "payMethod", _this.reviewData.payMethod);
        _this.$set(params, "fileBase64", _this.fileBase64);
        _this.$set(params, "fileName", _this.fileName);
        _this.$api.post(
          _this.$apis.manage + "/manage/order/financeReview/financeReview",
          params,
          function (res) {
            if (res.success) {
              _this.$message({
    ...
    check() {
      let _this = this;
      const checkPhone = /^((\d{11})|(\d{7,8})|(\d{4}|\d{3})-(\d{7,8}))$/;
      const checkNo = /^[0-9]*$/;
      if (_this.reviewData.payMethod == '3') {
        if (_this.accountNo == "") {
          _this.$message({
            message: "请填写账号",
            type: "error",
          ...
          if (!checkNo.test(_this.accountNo)) {
            _this.$message({
              message: "账号格式不正确",
              type: "error",
            });
        ...
        if (_this.phone == "") {
          ...
          if (!checkPhone.test(_this.phone)) {
            _this.$message({
              message: "联系方式格式不正确",
              type: "error",
            });
