    ...
    <div class="rh-drawer-body">
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="管理员审核" name="4">
          <el-row v-if="arry.length > 0">
            <el-col :span="24">
              <span class="name">
                数据源选择：<span style="color:red">(首次购买的产品需要配置数据源)</span>
              </span>
            </el-col>
          </el-row>
          <div class="space10" v-if="arry.length > 0"></div>
          <el-row v-if="arry.length > 0">
            <el-col :span="24">
  ...
  methods: {
    ...
    getDs() {
      ...
      if (_this.reviewData.orderType == "0") {
        ...
        _this.$set(params, "dbindepdent", _this.dbindepdent);
        if (params != null) {
        ...
        _this.$api.post(
          _this.$apis.manage + "/manage/order/adminReview/getDs",
          params,
          function (res) {
            if (res.success) {
              _this.arry = res.data;
    ...
    confirm() {
      ...
      if (arry.length > 0) {
        for (let i = 0; i < arry.length; i++) {
          if (arry[i].data.value == "") {
            _this.$message({
              message: "请为首次购买的产品配置数据源",
              type: "error",
            });
            return false;
          }
      ...
      _this.$set(params, "orderType", _this.reviewData.orderType);
      _this.$set(params, "operateIdea", _this.operateIdea2);
      _this.$set(params, "tenantId", _this.tenantId);
      _this.$set(params, "arry", _this.arry);
      _this.$api.post(
        _this.$apis.manage + "/manage/order/adminReview/adminReview",
        params,
        function (res) {
          if (res.success) {
            _this.$message({
