            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="预警类别" prop="warnType">
                  <el-select v-model.trim="operatorForm.warnType" placeholder="" disabled="disabled">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="预警等级" prop="warnLevelId">
                  <el-radio v-for="item in warnLevelList" :key="item.warnLevelId" v-model.trim="operatorForm.warnLevelId" :label="item.warnLevelId"  disabled="disabled">{{item.warnLevelName}}</el-radio>
...
export default { 
  ...
  data() {
    return {
      ...
      warnLevelList:[],
      operatorForm: { 
        warnConfigId:this.detailData. warnConfigId,
        warnConfigName:'',
        warnType:'',
        warnLevelId:'',
        warnDataBegin:'',
  ...
  created() {
    ...
    var warnConfigId = this.detailData. warnConfigId;
    this.initWarnSetting(warnConfigId);
  },
  ...
  methods: {
    ...
    initWarnSetting(warnConfigId){
      let _this = this;
      _this.$api.get(this.$apis.manage+"/rest/warn/warnsetting/"+warnConfigId, null, function (res) {
        if (res.success == true) {
          let list = res.data;
    ...
    save(){
      ...
      _this.$refs.operatorForm.validate((valid) => {
        if (valid) {
          ...
          _this.loading = true;
          _this.$api.put(this.$apis.manage+"/rest/warn/warnsetting/", para, function (res) {
            _this.loading = false;
            if (res.success == true) {
              _this.$message({
                message: '修改成功',
