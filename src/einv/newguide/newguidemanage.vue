    ...
    <div class="rh-page-header">
      <div class="rh-head-and-operate">
        <div class="tips-content">
          <span class="rh-helpTip-text">新手指导管理</span>
        </div>
      </div>
    </div>
    <div class="rh-page-content">
      ...
      <div class="rh-table">
        <el-table :data="tableData.records" stripe border  v-loading="tableLoading" @selection-change="handleSelectionChange">
          ...
          <el-table-column align="left" prop="title" label="标题" width="550"></el-table-column>
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              ...
              <el-button class="table_btn" type="text" v-if="scope.row.state=='0'" @click="doPublish(scope.row.newGuideId,'发布')">发 布</el-button>
              <el-button class="table_btn" type="text" v-if="scope.row.status=='N'" @click="enableNotyfy(scope.row.newGuideId, 'Y', '启用')">启用</el-button>
              <el-button class="table_btn" type="text" v-if="scope.row.status=='Y'" @click="enableNotyfy(scope.row.newGuideId, 'N', '禁用')">禁用</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <el-drawer :visible.sync="showmodel" size="73%" append-to-body :wrapperClosable = false :with-header=true>
        <newguideeditor v-if="showmodel" @back="back" @reload="init" :newguideinfo="newguideinfo"></newguideeditor>
      </el-drawer>
      <el-drawer :visible.sync="showDetail" size="73%" append-to-body :wrapperClosable = false :with-header=true>
        <newguidedetail v-if="showDetail" @back="back" @reload="init" :newguideinfo="newguideinfo"></newguidedetail>
      </el-drawer>
    </div>
  </div>
</template>
...
<script>
import newguideeditor from  './newguideeditor.vue'
import newguidedetail from  './newguidedetail.vue'
import validate from "@/utils/validate.js";

export default {
  name: 'editoritem',
  components: {newguideeditor, newguidedetail},
  data() {
    return {
      tableLoading: false,
      //查询表单
  ...
  methods: {
    ...
    init_Table() {
      ...
      _this.$api.post(this.$apis.manage+"/rest/manage/newguide/plist", params,
        function (res) {
        ...
        _this.$confirm('是否批量发布？', '提示', {
          ...
          let ids = _this.tr_sels.map(function (currentValue,index,arr){
              if(currentValue.state != 1){
                  return currentValue.newGuideId
              }
    ...
    doPublish(val,message){
      let _this = this;
      _this.$api.post(this.$apis.manage+"/rest/manage/newguide/publish",{'newGuideIds':val},function (res) {
        if(res.success == true) {
          _this.$message({
            message: message+'成功',
            type: 'success'
    ...
    enableNotyfy(newGuideId, status,message){
       let _this = this;
       var params = {
          newGuideId: newGuideId,
          status: status
       };
       if (params != null) {
            if (validate.checkForbiddenWordForm(JSON.stringify(params))) {
      ...
      _this.$api.post(this.$apis.manage+"/rest/manage/newguide/enablenotyfy",params,function (res) {
        if(res.success == true) {
          _this.$message({
            message: message+'成功',
            type: 'success'
