    ...
    <div class="rh-page-header">
      <div class="rh-head-and-operate">
        <div class="tips-content">
          <span class="rh-helpTip-text"> 我的订单 </span>
        </div>
      </div>
    </div>
    <div class="rh-page-content">
      <div class="tabs-wrap">
          ...
          <el-form :model="queryForm" ref="queryForm" :inline="false" label-width="0px" label-position="right">
            ...
            <el-form-item prop="payMethod">
              <el-select v-model="queryForm.payMethod" placeholder="支付方式">
                ...
                <el-option key="3" label="线下支付" value="3"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item prop="orderStatus">
              <el-select v-model="queryForm.orderStatus" placeholder="订单状态">
                ...
                <el-option key="1" label="已付款-待审核" value="1"></el-option>
                <el-option key="2" label="财务确认" value="2"></el-option>
                <el-option key="3" label="完成" value="3"></el-option>
                <el-option key="4" label="申请退款" value="4"></el-option>
                <el-option key="5" label="退款管理员审核" value="5"></el-option>
                <el-option key="6" label="已退款" value="6"></el-option>
                <el-option key="7" label="未付款关闭" value="7"></el-option>
                <el-option key="8" label="未付款无效订单" value="8"></el-option>
                <el-option key="9" label="开票中" value="9"></el-option>
                <el-option key="10" label="开票完成" value="10"></el-option>
              </el-select>
            </el-form-item>
        ...
        <el-table :data="tableData.records" stripe border height="400" v-loading="loading">
          ...
          <el-table-column show-overflow-tooltip align="center" prop="orderId" label="订单号" width="160"></el-table-column>
          ...
          <el-table-column show-overflow-tooltip align="center" prop="orderStatusName" label="订单状态" width="110"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="paymentTime" label="支付时间"></el-table-column>
          ...
          <el-table-column align="left" label="操作">
            <template slot-scope="scope">
              ...
              <el-button class="table_btn" type="text" v-if="scope.row.orderStatus == '0'" @click="payment(scope.row)">付款</el-button>
              <el-button class="table_btn" type="text" v-if="scope.row.orderStatus == '0'" @click="cancelOrder(scope.row.orderId)">取消订单</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    init_Table() {
      ...
      _this.loading = true;
      this.$api.post(
        _this.$apis.order + "/order/orderInfo/listOrder/",
        params,
        function (res) {
          if (res.success) {
    ...
    payment(row) {
      let _this = this;
      //微信未付款订单，点击付款时调用微信查询接口，判断订单是否支付成功
      _this.$api.get(
        ...
        function (res) {
          if (res.success) {
            ...
            _this.$router.push({
              ...
              query: {
                orderId: row.orderId,
                amountsPrice: row.orderTotalMoney
              }
            });
          }
        }
    ...
    details(row) {
      ...
      if (row.orderType == "0" || row.orderType == "2") {//0、产品订单2、续费订单
        _this.isProduct = true;
        _this.productData = {
          orderId: orderId,
          orderType: row.orderType,
        };
      } else if (row.orderType == "1") {//1、短信包订单
        _this.$api.post(
